...
<style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .edit-wrapper { padding: 24px 20px; max-width: 880px; margin: 0 auto; }
    ...
    .info-label { color: #9499a8; }
    .info-value { font-weight: 600; color: #374151; }

    /* Capacity bar */
    .cap-wrap { margin-top: 10px; }
    .cap-bar-bg { height: 6px; background: #f0f1f5; border-radius: 99px; overflow: hidden; margin: 4px 0; }
    .cap-bar    { height: 100%; border-radius: 99px; transition: width 0.4s, background 0.4s; }
    .cap-text   { font-size: 0.72rem; color: #9499a8; }
    ...
    @media (max-width: 900px) {
        ...
        .session-info { flex-direction: row; flex-wrap: wrap; gap: 10px; }
        .info-row { flex: 1 1 auto; flex-direction: column; gap: 1px; }
...
@php
    $insCount = $session->inscriptions->count();
    $cap      = $session->capacite ?: 1;
    $pct      = min(100, round(($insCount / $cap) * 100));
...
<div class="edit-wrapper">
    ...
    <div class="page-header">
        ...
        <div>
            <div class="page-title">Éditer la Session</div>
            <div class="page-sub">Mise à jour de la session <strong>#{{ str_pad($session->id, 4, '0', STR_PAD_LEFT) }}</strong></div>
        </div>
    </div>
    ...
    <div class="layout">
        ...
        <div class="preview-card">
            ...
            <div class="preview-calendar" id="previewCal">
                <div class="cal-month" id="previewMonth">
                    @if($session->date_debut)
                        {{ \Carbon\Carbon::parse($session->date_debut)->translatedFormat('M Y') }}
                    @else Période @endif
                </div>
                <div class="cal-range" id="previewRange">
                    {{ optional($session->date_debut)->format('d/m') ?? '—' }} →
                    {{ optional($session->date_fin)->format('d/m') ?? '—' }}
                </div>
                <div class="cal-days" id="previewDays">
                    @if($session->date_debut && $session->date_fin)
                        {{ \Carbon\Carbon::parse($session->date_debut)->diffInDays($session->date_fin) }} jours
                    @else — jours @endif
                </div>
            </div>
            ...
            <div class="preview-body">
                <div class="preview-formation">
                    ...
                    <span class="pf-text" id="previewFormation">{{ $session->formation->titre ?? '—' }}</span>
                </div>
                ...
                <div class="preview-meta">
                    <div class="pm-row">
                        ...
                        <span class="pm-value" id="previewLieu">{{ $session->lieu ?? '—' }}</span>
                    </div>
                    <div class="pm-row">
                        <span class="pm-label">Statut</span>
                        <span class="status-badge {{ $session->statut === 'ouverte' ? 'badge-ouverte' : 'badge-fermee' }}" id="previewStatus">
                            <span class="dot"></span>
                            <span id="previewStatusText">{{ ucfirst($session->statut) }}</span>
                        </span>
                    </div>
                </div>

                {{-- Inscriptions / Capacité --}}
                <div class="session-info">
                    <div class="info-row">
                        <span class="info-label">Inscrits</span>
                        <span class="info-value" style="color:#4F6EF7;">{{ $insCount }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Capacité</span>
                        <span class="info-value" id="previewCap">{{ $session->capacite }}</span>
                    </div>
                </div>

                <div class="cap-wrap">
                    <div class="cap-bar-bg">
                        <div class="cap-bar" id="previewBar"
                            style="width:{{ $pct }}%; background:{{ $barColor }};"></div>
                    </div>
                    <div class="cap-text">{{ $insCount }}/{{ $session->capacite }} places ({{ $pct }}%)</div>
                </div>
            </div>
        </div>
        ...
        {{-- Form Card --}}
        <div class="form-card">
            <form action="{{ route('admin.update-session', $session->id) }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    <label class="form-label" for="formation_id">Formation associée</label>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <input type="date" name="date_debut" id="date_debut"
                                ...
                                value="{{ old('date_debut', optional($session->date_debut)->format('Y-m-d')) }}" required>
                        </div>
                        ...
                        <div class="status-pills">
                            <div class="status-pill">
                                <input type="radio" name="statut" id="statut_ouverte" value="ouverte"
                                    {{ old('statut', $session->statut) == 'ouverte' ? 'checked' : '' }}>
                            ...
                            <div class="status-pill">
                                <input type="radio" name="statut" id="statut_fermee" value="fermee"
                                    {{ old('statut', $session->statut) == 'fermee' ? 'checked' : '' }}>
...
<script>
    ...
    function updatePreview() {
        ...
        previewFormation.textContent = fId && formations[fId] ? formations[fId] : '{{ $session->formation->titre ?? "—" }}';
        ...
        previewCap.textContent = capInput.value || '—';
        const pct = Math.min(100, Math.round((insCount / cap) * 100));
        const barColor = pct >= 80 ? '#ef4444' : (pct >= 50 ? '#f59e0b' : '#10b981');
        if (previewBar) { previewBar.style.width = pct + '%'; previewBar.style.background = barColor; }

        const checked = document.querySelector('input[name="statut"]:checked');
        const statut  = checked ? checked.value : '{{ $session->statut }}';
