<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-3">
            ...
            <select id="role" name="role" :class="'form-select' . ($errors->get('role') ? ' is-invalid' : '')"
                required>
                <option value="">{{ __('Select a role') }}</option>
                <option value="apprenant" {{ old('role') == 'apprenant' ? 'selected' : '' }}>{{ __('Apprenant') }}
                </option>
                <option value="formateur" {{ old('role') == 'formateur' ? 'selected' : '' }}>{{ __('Formateur') }}
                </option>
